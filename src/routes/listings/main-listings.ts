...
mainListings.post("/create_listing", async (req: Request, res: Response): Promise<void> => {
    ...
    const newListing: Listing = await Listing.create({ title: req.body.listing, description: req.body.description, bid: req.body.bid, dateOfCreation: findDate(), dateOfFinishing: "false", creator: authenticated.name, link: req.body.image, category: req.body.category, finished: false });
...
mainListings.get("/editlistings/:listing", async (req: Request, res: Response): Promise<void> => {
    const name: Array<Listing> = (await Listing.findAll({
        where: {
            id: parseInt(req.params.listing)
        }
    }))
    ...
    else if (name.length == 0 || authenticated.name != name[0].creator){
...
mainListings.post("/editlistings/:listing", async (req: Request, res: Response): Promise<void> => {
    await Listing.update({ description: req.body.description, link: req.body.link, category: req.body.category }, {
        where: {
            id: parseInt(req.params.listing)
        }
    })
