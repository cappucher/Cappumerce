...
mainListings.post("/create_listing", async (req, res) => {
    console.log(login_1.authenticated.name);
    const newListing = await models_1.Listing.create({ title: req.body.listing, description: req.body.description, bid: req.body.bid, dateOfCreation: findDate(), dateOfFinishing: "false", creator: login_1.authenticated.name, link: req.body.image, category: req.body.category, finished: false });
...
mainListings.get("/editlistings/:listing", async (req, res) => {
    ...
    else if (name.length == 0 || login_1.authenticated.name != name[0].creator) {
...
mainListings.post("/editlistings/:listing", async (req, res) => {
    await models_1.Listing.update({ description: req.body.description, link: req.body.link, category: req.body.category }, {
        where: {
            id: parseInt(req.params.listing)
        }
    });
